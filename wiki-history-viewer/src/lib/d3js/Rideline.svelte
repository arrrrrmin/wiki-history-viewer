<script lang="js">
    ...
    const getSeries = () => {
        ...
        return d3
            ...
            .map(([name, values]) => {
                const value = new Map(
                    values.map((d) => [+d.timestamp, Math.abs(d.delta)]),
                );
                return {
                    name,
    ...
    onMount(() => {
        ...
        z.domain([0, d3.mean(series, (d) => d3.max(d.values))]).range([
            0,
            -40, // -overlap * y.step(),
        ]);
        ...
        d3.select(`h3#${id}`).html(
            `Edits per user on '${props.result.title.replaceAll("_", " ")}' (${props.result.lang}-Wikipedia)`,
        );
        ...
        svg.append("g").attr("id", idHandler.register("g", "groups"));
